...
import { Loader2, ArrowLeft, PawPrint, Save } from "lucide-react";
...
const WritePrescription = () => {
  ...
  // Get consultation ID from URL params or state
  const consultationId = new URLSearchParams(location.search).get('consultationId') || location.state?.consultationId;
  ...
  useEffect(() => {
    const fetchConsultation = async () => {
      ...
      try {
        ...
        setPrescription(typedConsultation.prescription || "");
        setLoading(false);
      } catch (error) {
        console.error("Error fetching consultation:", error);
        toast({
  ...
  const handleSavePrescription = async () => {
    if (!consultation || !prescription.trim()) {
      toast({
        title: "Error",
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Prescription saved successfully and consultation marked as completed.",
      });
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm bg-muted p-3 rounded-md">{consultation.symptoms}</p>
              </div>
              
              <div>
                <h4 className="font-medium text-sm text-muted-foreground mb-1">Submitted</h4>
                <p className="text-sm">{new Date(consultation.created_at).toLocaleString()}</p>
              </div>
            </CardContent>
          </Card>
